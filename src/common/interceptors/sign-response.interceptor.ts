...
/**
 * Wraps the response body in `{ [tokenName]: <signed JWT> }`.
 * Honours a `Do-Not-Sign: true` request header in non-production
 * environments to ease local debugging.
 */
@Injectable()
export class SignResponseInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    const opts = this.reflector.get<SignResponseOptions>(SIGN_RESPONSE_KEY, context.getHandler());
    if (!opts) {
      return next.handle();
    }
