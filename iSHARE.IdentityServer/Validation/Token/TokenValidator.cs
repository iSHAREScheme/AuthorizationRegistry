...
using iSHARE.IdentityServer.Validation.Token.Logging;
using Microsoft.AspNetCore.Authentication;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.Logging;
using Microsoft.IdentityModel.Tokens;

namespace iSHARE.IdentityServer.Validation.Token
{
    /// <summary>
    /// Stolen from IdentityServer4 code and tweaked it according to iSHARE needs.
    /// iSHARE contains custom issuer which is party EORI number, IdentityServer4 doesn't allow to override issuer, so validation always kept failing.
    ///
    /// If IdentityServer4 in the future updates decides to provide a possibility to inject custom validation for the following fields,
    /// then this class and the whole <see cref="iSHARE.IdentityServer.Validation.Token"/> namespace can be removed.
    /// This copy pasta currently overrides the following values, remaining validation logic is the same as in IdentityServer4 package:
    /// "iss" - instead of URI, party EORI is used
    /// </summary>
    internal class TokenValidator : ITokenValidator
    {
        ...
        public async Task<TokenValidationResult> ValidateAccessTokenAsync(string token, string expectedScope = null)
        {
            ...
            if (result.IsError)
            {
                LogError("Error validating JWT access token.");
                _logger.LogError("Validation return returned an error: {error}.", result.Error);
                return result;
            }
        ...
        private async Task<TokenValidationResult> ValidateJwtAsync(string jwt, IEnumerable<SecurityKeyInfo> validationKeys, bool validateLifetime = true, string audience = null)
        {
            ...
            {
                ValidIssuer = _partyDetailsOptions.ClientId,
            ...
            try
            {
                ...
                if (audience.IsMissing())
                {
                    if (_options.AccessTokenJwtType.IsPresent())
                    {
                        ...
                        {
                            _logger.LogError("Expected token type to be {expected}, actual was {actual}.", _options.AccessTokenJwtType, type);
        ...
        private IEnumerable<Claim> ReferenceTokenToClaims(IdentityServer4.Models.Token token)
        {
